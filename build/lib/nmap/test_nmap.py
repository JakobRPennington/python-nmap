...
def xmlfile_read_setup():
    nm.analyse_nmap_xml_scan(open('test/scanme_output.xml').read())
...
def test_command_line():
    ...
    assert_equals(nm.command_line(), './nmap-{0}/nmap -sV -oX test/scanme_output-{0}.xml scanme.nmap.org'.format(NMAP_XML_VERSION))
...
def xmlfile_read_setup_multiple_osmatch():
    nm.analyse_nmap_xml_scan(open('test/osmatch_output.xml').read())
...
def test_convert_nmap_output_to_encoding():
    a=nm.analyse_nmap_xml_scan(open('test/scanme_output.xml').read())
